...
import {Activity, ArrowLeft, Download, Trophy, Zap, Target} from "lucide-react";
...
export default function Squats({ user, onFinish }) {
    ...
    const reset = useCallback(async () => {
        ...
        // Ensure everything else is stopped
        await stopEverything();
        setFile(null);
        setUseCamera(true);
        setSummary(null);
    ...
    // FRT Handlers
    const handleFrtRatingChange = useCallback((e) => {
        setFrtRating(parseInt(e.target.value));
    }, []);

    const handleResetToModerate = useCallback(() => {
        setFrtRating(50);
    ...
    const handlePersonalizedPlan = useCallback(() => {
        alert("Redirecting to payment gateway for personalized fitness plans...");
    }, []);
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex items-center gap-3">
                                    <div className="p-3 rounded-xl bg-green-500/30 border border-green-500/20">
                            ...
                            <div className="space-y-4">
                                <label className="group cursor-pointer">
                                    <div className={`flex items-center p-4 rounded-xl border-2 transition-all duration-300 ${
                                        useCamera === true ? 'border-primary bg-primary/10 shadow-lg shadow-primary/20' : 'border-border/50 hover:border-primary/30 hover:bg-primary/5'
                                    }`}>
                                        <div className={`w-6 h-6 rounded-full border-2 mr-4 flex items-center justify-center transition-all duration-300 ${
                                            useCamera === true ? 'border-primary bg-primary' : 'border-border/50 group-hover:border-primary/50'
                                        }`}>
                                            {useCamera === true && (
                                    ...
                                    <input type="radio" checked={useCamera === true} onChange={handleCameraSelect} className="sr-only" />
                                </label>

                                <label className="group cursor-pointer">
                                    <div className={`flex items-center p-4 rounded-xl border-2 transition-all duration-300 ${
                                        useCamera === false ? 'border-primary bg-primary/10 shadow-lg shadow-primary/20' : 'border-border/50 hover:border-primary/30 hover:bg-primary/5'
                                    }`}>
                                        <div className={`w-6 h-6 rounded-full border-2 mr-4 flex items-center justify-center transition-all duration-300 ${
                                            useCamera === false ? 'border-primary bg-primary' : 'border-border/50 group-hover:border-primary/50'
                                        }`}>
                                            {useCamera === false && (
                                                <div className="w-3 h-3 rounded-full bg-white animate-pulse"></div>
                                            )}
                                    ...
                                    <input type="radio" checked={useCamera === false} onChange={handleUploadSelect} className="sr-only" />
                                </label>
                            </div>
                            ...
                            {useCamera === false && !file && (
                                <div className="mt-6">
                                    <div className="relative">
                                        <input type="file" accept="video/*" onChange={handleFileSelect} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 gap-4">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <Button onClick={stopSession} disabled={!running} variant="outline" className="h-12 bg-red-500/10 border-red-500/30 text-red-400 hover:bg-red-500/20 hover:border-red-500/50 font-semibold transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed">
                                        <div className="flex items-center gap-2">
                                            <div className="w-2 h-2 rounded-full bg-red-400"></div>
                                            <span>Stop</span>
                                        </div>
                                    </Button>
                                    <Button onClick={reset} variant="outline" className="h-12 bg-gray-500/10 border-gray-500/30 text-gray-400 hover:bg-gray-500/20 hover:border-gray-500/50 font-semibold transition-all duration-300">
                                        <div className="flex items-center gap-2">
                                    ...
                                    <div className="relative p-8 pb-12 h-full flex flex-col">
                                        ...
                                        <div className="space-y-10 flex-1">
                                            <div className="group/step flex items-start gap-5 p-5 rounded-2xl bg-gradient-to-br from-primary/15 to-primary/5 border border-primary/25 hover:border-primary/40 hover:shadow-lg transition-all duration-300">
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <p className="text-sm text-muted-foreground leading-relaxed">
                                                        Stand with feet shoulder-width apart, chest up, core tight.
                                                    </p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="group/step flex items-start gap-5 p-5 rounded-2xl bg-gradient-to-br from-primary/15 to-primary/5 border border-primary/25 hover:border-primary/40 hover:shadow-lg transition-all duration-300">
                                                ...
                                                <div className="flex-1">
                                                    <h4 className="font-semibold text-foreground mb-2 text-lg">Rise</h4>
                                                    <p className="text-sm text-muted-foreground leading-relaxed">
                                                        Press through heels, straighten legs, return to standing.
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-2 gap-5">
                                            ...
                                            <div className="group/stat text-center p-6 rounded-2xl bg-gradient-to-br from-accent/15 to-accent/5 border border-accent/25 hover:border-accent/40 hover:shadow-lg transition-all duration-300">
                                                <div className="text-3xl font-bold bg-gradient-to-r from-foreground to-primary bg-clip-text text-transparent mb-2">
                                                    {summary ? `${summary.total_duration}s` : startTimeRef.current ? Math.round((performance.now() / 1000) - startTimeRef.current) + "s" : "0s"}
                                                </div>
